...
/**
 * Servlet implementation class AddSupplierShipServlet
 */
@WebServlet("/AddSupplierShipServlet")
public class AddSupplierShipServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddSupplierShipServlet() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String SupplierID = request.getParameter("sup_id");
		...
		//Prepared Statement to add student data
 		PreparedStatement checkId = con .prepareStatement("SELECT COUNT(*) FROM sup_trans_info WHERE TransactionID = ?;");
 		...
 		if(ret.getInt("COUNT(*)") != 0) {
 			...
 			PreparedStatement st = con .prepareStatement("insert into sup_trans_info values(?, ?);");
 	 		st.setInt(1,Integer.parseInt(id));
 			st.setInt(2,Integer.parseInt(SupplierID));
 			int result=st.executeUpdate();

 			//Checks if insert is successful.If yes,then redirects to Result.jsp page 
 			if(result>0)
 			{
 				PreparedStatement sttemp = con .prepareStatement("insert into sup_payment values(?, ? ,?, ?)");
